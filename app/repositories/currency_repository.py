...
from app.schemas.currency import CurrenciesList, CurrencyWithoutRate, LastCurrenctUpdate
...
class CurrencyLastUpdateRepository:
    ...
    async def update_or_create(self) -> None:
        async with self.session_factory() as session:
            ...
            if not last_currency_update_obj:
                last_currency_update_obj = self._model(last_update=datetime.now())
            else:
    ...
    async def get_last_currency_update(self) -> LastCurrenctUpdate:
        async with self.session_factory() as session:
            ...
            return LastCurrenctUpdate(last_update=last_currency_update_obj.last_update)
