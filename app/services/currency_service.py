...
from app.repositories.currency_repository import CurrencyRepository, CurrencyLastUpdateRepository
from app.schemas.currency import (
    CustomResp, CurrencyWithoutRate, 
    LastCurrenctUpdate, ConvertCurrencies, ConvertedCurrencyAmount
)
from app.schemas.responses import CurrenciesListResponse
from app.exceptions.errors import BaseError, DoesNotExistsError
...
class CurrencyService:
    ...
    async def create_currencies(self) -> None:
        try:
            currencies = await self._currency_exchangerates_api_client.fetch_all_currencies()
    ...
    async def update_currencies(self):
        try:
            currencies = await self._currency_exchangerates_api_client.fetch_currencies_rates()
        except Exception as error:
            raise error
    ...
    async def get_last_currency_update(self) -> LastCurrenctUpdate:
        try:
            return await self._currency_last_update_repository.get_last_currency_update()
        except Exception as error:
            raise error
    ...
    async def convert_currencies(
        ...
        converted_amount = body.amount * float(target_currency_rate / source_currency_rate)
